...
for str in spilt_list: 
    if (not str.find('NF') == -1) or (not str.find('解锁') == -1): 
...
proxy_groups = proxy_groups + "    - { name: Apple, type: select, proxies: [代理节点, DIRECT]}\n"
proxy_groups = proxy_groups + "    - { name: Medias, type: select, proxies: [媒体节点, 代理节点, DIRECT]}\n"
proxy_groups = proxy_groups + "    - { name: China-Websites, type: select, proxies: [DIRECT, 代理节点]}\n"
proxy_groups = proxy_groups + "    - { name: Final, type: select, proxies: [代理节点, DIRECT]}\n"
proxy_groups = proxy_groups + "    - { name: --------------------------------------------, type: select, proxies: [DIRECT]}\n"
proxy_groups = proxy_groups + "    - { name: Auto, type: url-test, proxies: [" + origin_string + "], url: 'http://www.gstatic.com/generate_204', interval: 86400 }\n"
proxy_groups = proxy_groups + "    - { name: Auto-Medias, type: url-test, proxies: [" + filtered_string + "], url: 'http://fast.com/', interval: 86400 }\n"
proxy_groups = proxy_groups + "    - { name: Fallback, type: fallback, proxies: [" + origin_string + "], url: 'http://www.gstatic.com/generate_204', interval: 7200 }\n"
